...
void show_device_info(const char *device, int port_number, int verbose)
{
    ...
    if (verbose)
    {
        if (hwinfo->flags & EXANIC_HW_FLAG_DRAM_VARIANT)
        {
            ...
            printy_kv(2,"DDR DRAM", "%s", ddr_fitted ? "present" : "not present");
        }
    ...
    if (hwinfo->hwid != -1)
    {
        ...
        if(!yaml_out)
        {
            printy_kv(2,"Temperature", "%.1f C   VCCint: %.2f V   VCCaux: %.2f V",
		   temp_real, vccint_real, vccaux_real);
        }
        else
        {
            printy_kv(2,"Temperature", "%.1f C", temp_real);
            printy_kv(2,"VCCint", "%.2f V", vccint_real);
            printy_kv(2,"VCCaux", "%.2f V", vccaux_real);
        }
    ...
    if (hwinfo->flags & EXANIC_HW_FLAG_FAN_RPM_SENSOR)
    {
        ...
        printy_kv(2,"Fan speed", "%.0f RPM", rpm);
    }

    str = exanic_function_id_str(function);
    printy_kv(2,"Function", "%s", (str == NULL) ? "unknown" : str);

    {
        ...
        printy_kv(2,"Firmware date", "%04d%02d%02d (%s)", tm->tm_year + 1900,
    ...
    if (function == EXANIC_FUNCTION_DEVKIT)
    {
        ...
        printy_kv(2,"Customer version", "%u (%x)", user_version, user_version);
    ...
    if (hwinfo->flags & EXANIC_HW_FLAG_PWR_SENSE)
    {
        ...
        printy_kv(2,"External 12V power", "%s", ext_pwr ? "detected" : "not detected");
    }
        ...
        if (hw_type != EXANIC_HW_X4 && hw_type != EXANIC_HW_X2)
        {
            ...
            if (pps_out)
            {
                if (config & EXANIC_HW_PER_OUT_CONFIG_PPS)
                  printy_kv(2,"PPS port", "1PPS output, on %s edge\n",
                            (flags & EXANIC_HW_SERIAL_PPS_OUT_VAL) ? "rising" : "falling");
                else if (config & EXANIC_HW_PER_OUT_CONFIG_10M)
                  printy_kv(2,"PPS port", "10MHz output\n");
                else
                    printf( "disabled\n");
            }
            else
            {
        	    printy_kv(2,"PPS port", "input, termination %s", pps_term_en ? "enabled" : "disabled");
            }
        }
    }
    ...
    if (function == EXANIC_FUNCTION_NIC)
    {
            ...
            hw_type == EXANIC_HW_X4 || hw_type == EXANIC_HW_X2)
        {
            ...
            printy_kv(2,"Bridging", "%s", (pl_cfg & EXANIC_FEATURE_BRIDGE) ?
    ...
    if (function == EXANIC_FUNCTION_FIREWALL)
    {
        ...
        printy_kv(2,"Firewall capability", "%s",(fw_capable) ? "supported" :
            "unsupported");
    }
    ...
    for (i = first_port; i <= last_port; i++)
    {
        ...
        {
            printy_kv(4,"Port status", "%s, %s, %s, %s",
                    (port_status & EXANIC_PORT_STATUS_ENABLED) ?
                        "enabled" : "disabled",
                    (port_status & EXANIC_PORT_STATUS_SFP) ?
                        "SFP present" : "no SFP",
                    ...
                    && rx_usable)
        {
            ...
            if (verbose)
            {
		        ...
		        {
                    printy_kv(4,"MAC filters", "%d", mac_rules);
                    printy_kv(4,"IP filters", "%d", ip_rules);
		        }
                printy_kv(4,"TX buffer size", "%dkB", tx_size);
            }

            loopback = get_local_loopback(exanic, i);
            if ((loopback != -1) && (loopback || verbose))
                printy_kv(4,"Loopback mode", "%s", loopback ? "on" : "off");

            promisc = exanic_get_promiscuous_mode(exanic, i);
            if ((promisc != -1) && (promisc || verbose))
                printy_kv(4,"Promiscuous mode","%s", promisc ? "on" : "off");
        }
                ...
                && strlen(ifname) > 0)
        {
            ...
            if ((bypass != -1) && (bypass || verbose))
                printy_kv(4,"Bypass-only mode","%s", bypass ? "on" : "off");
        }
                    ...
                    && (rx_usable || tx_usable))
        {
            ...
            {
                printy_kv(4,"MAC address","%02x:%02x:%02x:%02x:%02x:%02x",
		    ...
		    {
                        printy_kv(4,"IP address", "%s", inet_ntoa(address));
                        printy_kv(4,"IP address mask", "%s", inet_ntoa(netmask));
                    }
                }
            }
        }
	    ...
	    {
  	      printy_kv(4,"RX packets", "%u", port_stats.rx_count);
	      printy_kv(4,"RX ignored", "%u", port_stats.rx_ignored_count);
	      printy_kv(4,"RX errors",  "%u", port_stats.rx_error_count);
	      printy_kv(4,"RX dropped", "%u", port_stats.rx_dropped_count);
	    }

            printy_kv(4,"TX packets","%u", port_stats.tx_count);
        }
    }
